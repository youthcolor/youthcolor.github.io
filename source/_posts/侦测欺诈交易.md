...
title: 侦测欺诈交易
categories: R
tags: [欺诈侦测,聚类,半监督分类,类失衡,贝叶斯分类器,AdaBoost分类器,决策精确度,回溯精确度,交叉验证,离群值侦测排序]
---
本案例来自葡萄牙作者Luis Torgo所著的《数据挖掘与Ｒ语言》一书第４章：侦测欺诈交易。

本章介绍了一类数据挖掘问题：离群值排序。特别是采用了一个从不同的方面处理这个任务的数据集。即应用了有监督的、无监督的和半监督的方法。对于应用有限的资源来找到一个现象的异常观测值这一常见问题，本章的应用可以看做是它的一个实例。多个实际问题可以映射到这个通用框架中，例如信用卡、电信和税收的欺诈侦测等。在证券领域，也有这种欺诈侦测一般概念的多个应用。
...
barplot(totS,main='Transactions per salespeople',names.arg='',xlab='Salespeople',
        ylab='Amount')
```
![image](http://wx4.sinaimg.cn/mw690/e621a9abgy1fiskqh3tsqj20gx09kjrb.jpg)
```r
barplot(totP,main='Transactions per product',names.arg='',xlab='Products',
        ylab='Amount')
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqhla0aj20gx09kjrd.jpg)
```r
...
```
![image](http://wx1.sinaimg.cn/mw690/e621a9abgy1fiskqi2syfj20gx0c3749.jpg)
```r
...
```
![image](http://wx1.sinaimg.cn/mw690/e621a9abgy1fiskqio0tij20gx0c3dfy.jpg)
```r
dms <- scale(ms)
smalls <- which(table(Prod) < 20)
...
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqopamzj20m80dzaa2.jpg)
...
```
![image](http://wx1.sinaimg.cn/mw690/e621a9abgy1fiskqpbb4dj20m80dzglr.jpg)
...
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqjbi7nj20m80g0glr.jpg)
...
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqjq9tbj20m80g0weq.jpg)
...
legend('bottomright',c('BPrule','LOF','ORh'),
       lty=c(1,2,1),col=c('black','black','grey'))
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqkjbjrj20m80g03yt.jpg)
...
```
![image](http://wx3.sinaimg.cn/mw690/e621a9abgy1fiskqkx1s0j20m80g0aai.jpg)
...
legend('bottomright',c('NaiveBayes','ORh'),
       lty=1,col=c('black','grey'))
```
![image](http://wx2.sinaimg.cn/mw690/e621a9abgy1fiskqlig5ij20m80g074j.jpg)
...
legend('bottomright',c('NaiveBayes','smoteNaiveBayes','ORh'),
       lty=c(1,2,1),col=c('black','black','grey'))
```
![image](http://wx4.sinaimg.cn/mw690/e621a9abgy1fiskqlymwnj20m80dzq37.jpg)
...
legend('bottomright',c('NaiveBayes','ORh','AdaBoostM1'),
       lty=c(1,1,2),col=c('black','grey','black'))
```
![image](http://wx1.sinaimg.cn/mw690/e621a9abgy1fiskqmoicdj20m80dzjrn.jpg)
...
legend('bottomright',c('NaiveBayes','ORh','NaiveBayes-ST'),
       lty=c(1,1,2),col=c('black','grey','black'))
```
![image](http://wx3.sinaimg.cn/mw690/e621a9abgy1fiskqnmuo1j20m80dzt90.jpg)
...
legend('bottomright',c('AdaBoostM1','ORh','AdaBoostM1-ST'),
       lty=c(1,1,2),col=c('black','grey','black'))
```
![image](http://wx3.sinaimg.cn/mw690/e621a9abgy1fiskqofvsmj20m80dz0t0.jpg)